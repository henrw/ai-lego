...
const Canvas = () => {
    ...
    const unselectCard = useCanvasStore(store => store.actions.unselectCard);
    ...
    return (
        ...
        <div className="flex flex-row w-full h-full" id='canvas'
            onClick={(e) => { if (e.target.id == 'canvas') unselectCard() }}
        >
            {cardsData.map((card) => (
                <Card
                    id={card.id} handleDelete={() => { }}
                    key={card.id}
                    text={card.descr}
            ...
            <div className='flex flex-col right-[1%] bottom-[1%] absolute'>
                ...
                <button onClick={() => addCardData("data")} className='bg-data rounded-lg p-2 my-2'>Data</button>
                <button onClick={() => addCardData("model")} className='bg-model rounded-lg p-2 my-2'>Model</button>
                <button onClick={() => addCardData("train")} className='bg-train rounded-lg p-2 my-2'>Train</button>
                <button onClick={() => addCardData("test")} className='bg-test rounded-lg p-2 my-2'>Test</button>
